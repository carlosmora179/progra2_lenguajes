...
<html>
    ...
    <body >
        ...
        <%! String toInsert = "";%>
        <%
            ConsultaMascota resultado = new ConsultaMascota();
            
            dueños = resultado.consultaGeneral();
            
            
            
            if(request.getParameter("cedula_propietario").isEmpty()){
                for(int x = 0; x < dueños.size(); x++){
                for(int z = 0; z < dueños.get(x).getMascotas().size(); z++){
                    toInsert += dueños.get(x).getMascotas().get(z).toString() + 
                            '-' + dueños.get(x).getNombre() + ' ' + dueños.get(x).getApellido1() + '_';
                }
            }
            
            }else{
                System.out.println(Integer.parseInt(request.getParameter("cedula_propietario")));
                
                for(int x = 0; x < dueños.size(); x++){
                    System.out.println(dueños.get(x).getCedula());
                    if(dueños.get(x).getCedula().compareTo(Integer.parseInt(request.getParameter("cedula_propietario"))) == 0){
                        for(int z = 0; z < dueños.get(x).getMascotas().size(); z++){
                            toInsert += dueños.get(x).getMascotas().get(z).toString() + 
                            '-' + dueños.get(x).getNombre() + ' ' + dueños.get(x).getApellido1() + '_';
                        }
